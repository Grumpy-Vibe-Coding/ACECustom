...
namespace ACE.Server.Managers
{
    ...
    public static class ServerConfig
    {
        ...
        public static ConfigProperty<long> room_assign_logout_hold_minutes { get; private set; } = new(15, "Room Assign: minutes a one-player room stays held for a character who logged out inside it after being handed a room (nobody else is sent there meanwhile). 0 = no logout holds, max 10080. Change live: /modifylong room_assign_logout_hold_minutes <minutes>");
        public static ConfigProperty<long> room_assign_leave_hold_seconds { get; private set; } = new(60, "Room Assign: seconds a one-player room stays held for a player who teleported out of it (recall, portal, death), so they return to the same room. 0 = no leave holds, max 86400. Change live: /modifylong room_assign_leave_hold_seconds <seconds>");
        public static ConfigProperty<long> room_assign_logout_hold_renewals { get; private set; } = new(1, "Room Assign: extra logout holds a character may take after the first, before it must be handed a room fresh again (a first-free room, not a return to its own or held room). Stops relogging to keep a room forever; 1 still covers crashing twice. 0-100. Change live: /modifylong room_assign_logout_hold_renewals <count>");
        public static ConfigProperty<long> room_assign_startup_grace_minutes { get; private set; } = new(15, "Room Assign: holds live in memory and a restart wipes them, so for this many minutes after the server starts a character logging in inside a free room keeps it (first to log in wins). 0 = off, max 1440. Change live: /modifylong room_assign_startup_grace_minutes <minutes>");
