...
namespace ACE.Server.Managers
{
    ...
    public static class ServerConfig
    {
        ...
        public static ConfigProperty<bool> missile_power_bar { get; private set; } = new(true, "If TRUE, the missile accuracy bar scales DAMAGE along the missile power ladder: missile_power_fast (bar 0, default 1.0), missile_power_mid (bar 0.5, default 0 = the average of fast and full) and missile_power_full (bar 1, default 2.5), each clamped to 0.1-10 at use and editable live with /missilepower <fast> <full> [mid]; attack skill keeps a 1.0x-1.6x accuracy bonus across the bar. If FALSE, retail behavior: the bar scales attack skill 0.6x-1.6x and damage is flat. Applies live, no restart. Use /modifybool missile_power_bar <true/false>.");
        /// <summary>Missile power ladder (owner 2026-09-12): damage multiplier at the FAST end of the accuracy bar
        /// (bar = 0). Only read while missile_power_bar is TRUE. Edit with /missilepower.</summary>
        public static ConfigProperty<double> missile_power_fast { get; private set; } = new(1.0, "Missile power ladder: damage multiplier at full speed (bar 0). Active only while missile_power_bar is TRUE. Edit with /missilepower <fast> <full> [mid].");
        /// <summary>Missile power ladder: multiplier at FULL draw (bar = 1).</summary>
        public static ConfigProperty<double> missile_power_full { get; private set; } = new(2.5, "Missile power ladder: damage multiplier at full draw (bar 1). Active only while missile_power_bar is TRUE. Edit with /missilepower <fast> <full> [mid].");
        /// <summary>Missile power ladder: multiplier at MID bar (0.5). 0 = unset = the average of fast and full (a straight line).</summary>
        public static ConfigProperty<double> missile_power_mid { get; private set; } = new(0.0, "Missile power ladder: damage multiplier at mid bar (0.5). 0 = the average of fast and full (straight line). Active only while missile_power_bar is TRUE. Edit with /missilepower <fast> <full> [mid].");
        ...
        public static ConfigProperty<bool> damage_event_debug_server_log { get; private set; } = new(false, "If TRUE, logs extensive melee/missile DamageEvent detail to the server log when the defender is a Player or CombatPet (incoming damage only). For pet→mob outgoing hits use pet_combat_outgoing_damage_event_debug_log. /modifybool damage_event_debug_server_log.");
        ...
        /// <summary>Verbose melee/missile <see cref="ACE.Server.Entity.DamageEvent"/> lines in server log for PLAYER attackers hitting non-player creatures — the outgoing case the defender-side logger excludes. Weapon-scaling tuning aid. Default FALSE.</summary>
        public static ConfigProperty<bool> damage_event_debug_player_vs_mob { get; private set; } = new(false, "If TRUE, logs extensive melee/missile DamageEvent detail to the server log when a Player attacks a non-player creature (outgoing damage; weapon tuning aid). /modifybool damage_event_debug_player_vs_mob.");
        /// <summary>Cap on PLAYER physical crit damage as a multiple of a max normal hit (all flats included). Bows' multiplied damage slice made their crits land ~7x a normal hit (owner 2026-08-01: tone down). 0 = uncapped. Mob crits are governed by the zone crit system instead.</summary>
        // player_crit_damage_cap DELETED 2026-08-29 (owner; it was our own 2026-08-01 addition, not
        // upstream ACE): the unified crit model makes the Crushing Blow band the ratio's bound by
        // construction, so the clamp had no read sites left. Any stored config row is an ignored
        // orphan.
        ...
        /// <summary>If TRUE, server physics blocks pet movement vs doors, monsters, and non-player objects while Ethereal allows client player walk-through. /modifybool pet_block_nonplayer_collision.</summary>
        public static ConfigProperty<bool> pet_block_nonplayer_collision { get; private set; } = new(true, "If TRUE, server physics blocks pet movement vs doors, monsters, and non-player objects while Ethereal allows client player walk-through. /modifybool pet_block_nonplayer_collision false");
        ...
        public static ConfigProperty<bool> pet_device_pyreal_auto_refill_enabled { get; private set; } = new(false, "If TRUE, players with PetDevicePyrealAutoRefillEnrolled may pay pyreals to refill empty summoning essence charges when summoning.");
        public static ConfigProperty<bool> pet_charm_universal_summoning_mastery_enabled { get; private set; } = new(true, "If TRUE, players with Universal Summoning Mastery charm active may use pet devices of any SummoningMastery (Primalist / Necromancer / Naturalist). Toggle: /modifybool pet_charm_universal_summoning_mastery_enabled false");

        // Room Assign (RoomAssignManager): holds live in memory only - a server restart clears them. Negative counts as 0.
        public static ConfigProperty<long> room_assign_logout_hold_minutes { get; private set; } = new(15, "Room Assign: minutes a one-player room stays held for the ACCOUNT of a character who logged out inside it (nobody else is sent there meanwhile). 0 = no logout holds, max 10080. Change live: /modifylong room_assign_logout_hold_minutes <minutes>");
        public static ConfigProperty<long> room_assign_leave_hold_seconds { get; private set; } = new(60, "Room Assign: seconds a one-player room stays held for the ACCOUNT of a player who teleported out of it (recall, portal, death), so they return to the same room. 0 = no leave holds, max 86400. Change live: /modifylong room_assign_leave_hold_seconds <seconds>");
        public static ConfigProperty<long> room_assign_logout_hold_renewals { get; private set; } = new(1, "Room Assign: extra logout holds an ACCOUNT may take after the first, before it must be handed a room fresh again (a first-free room, not a return to its own or held room). Stops relogging to keep a room forever; 1 still covers crashing twice. 0-100. Change live: /modifylong room_assign_logout_hold_renewals <count>");
        public static ConfigProperty<long> room_assign_startup_grace_minutes { get; private set; } = new(15, "Room Assign: holds live in memory and a restart wipes them, so for this many minutes after the world first opens a character logging in inside a free room keeps it (first to log in wins). 0 = off, max 1440. Change live: /modifylong room_assign_startup_grace_minutes <minutes>");
        ...
        public static ConfigProperty<double> pet_summon_lifespan_seconds_per_aug { get; private set; } = new(0.0, "Additive seconds per luminance Summon aug on pet summon (stacked with duration term when both > 0). Set 0 if only spell-duration scaling is desired.");
        public static ConfigProperty<bool> pet_combat_lifespan_duration_aug_ignore_summon_cap { get; private set; } = new(false, "If TRUE, duration-based lifespan bonus uses full luminance Spell Duration aug count × pet_combat_lifespan_seconds_per_duration_aug. If FALSE (default), uses min(Summon, Duration) counts so duration cannot exceed summon investment.");
        public static ConfigProperty<double> pet_combat_lifespan_seconds_per_duration_aug { get; private set; } = new(0.0, "Seconds added per effective spell-duration luminance aug (full count if pet_combat_lifespan_duration_aug_ignore_summon_cap; else min(Summon, Duration)). Additive with pet_summon_lifespan_seconds_per_aug.");
        public static ConfigProperty<long> pet_combat_recall_block_after_damage_seconds { get; private set; } = new(0, "Seconds after combat pet damage during which essence stow/recall is blocked and catch-up follow is delayed while still engaged nearby; owner-recall distance still drops target if owner kites away (0 = off).");
        ...
        public static ConfigProperty<bool> ring_aoe_los_indoor { get; private set; } = new(true, "if TRUE, player ring AOE (hand-cast, Rocky Shrapnel, Ring of Agony, Cast on Strike ring procs, Explosive Arrow) only hits targets in line of sight of the blast when the blast or the target is indoors (dungeon, building interior, basement). Walls, closed doors and solid objects block. The creature whose hit triggered a proc ring is always hit. Applies live. Use /modifybool ring_aoe_los_indoor <true/false>.");
        public static ConfigProperty<bool> ring_aoe_los_ignore_ground { get; private set; } = new(false, "if TRUE, ring line of sight (ring_aoe_los_indoor / ring_aoe_los_outdoor) ignores the ground outdoors, so hills never block a ring; buildings, doors, walls and objects still do. FALSE (default) = the ground blocks a ring the same way it blocks a streak. Applies from the next cast. Use /modifybool ring_aoe_los_ignore_ground <true/false>.");
        public static ConfigProperty<bool> ring_aoe_los_outdoor { get; private set; } = new(true, "if TRUE, player ring AOE only hits targets in line of sight of the blast when both the blast and the target are outdoors. Buildings, closed doors, rocks, trees and other solid objects block, and so do hills unless ring_aoe_los_ignore_ground is TRUE. The creature whose hit triggered a proc ring is always hit. Applies live. Use /modifybool ring_aoe_los_outdoor <true/false>.");
        ...
        public static ConfigProperty<bool> variation_interaction_diag_verbose { get; private set; } = new(false, "If TRUE, emits WARN-level [VariationInteraction] traces for: TrackObject/AddTrackedObject variation blocks, FindObject visibility blocks, UseItem/UseWithTarget player resolution, and trade open/close. Toggle live: /modifybool variation_interaction_diag_verbose true");
        ...
        public static ConfigProperty<bool> prompt_add_augment_economy_console_debug { get; private set; } = new(false, "If TRUE, prints verbose GetPromptAddAugmentEconomy diagnostics to console and DEBUG log (world DB host/database, full weenie emote dump, per-action type/msg compatibility, match candidates, chosen template). Toggle live: /modifybool prompt_add_augment_economy_console_debug true");
        ...
        public static ConfigProperty<bool> spawn_diag_verbose { get; private set; } = new(false, "If TRUE, emits WARN-level [SpawnDiag] traces when an object fails to spawn/place: get_landcell NULL, enter_world FAILED (incl. sposErr placement error), and AddWorldObject/CreateWorldObjects add failures. Default OFF -- when off the entire diag path (string building included) is skipped, so there is zero overhead. Turn on only when investigating spawn failures. Toggle live: /modifybool spawn_diag_verbose true");
        public static ConfigProperty<bool> regen_diag_verbose { get; private set; } = new(false, "If TRUE, emits WARN-level [RegenDiag] lines for every PLAYER vital regen tick evaluation (health/stamina/mana below max): current/max, regen rate, attribute/stance/enchantment/aug modifiers, whether the Zone Control Suppression card blocked the Prodigal regen line, the zone regen multiplier, and the resulting tick. Use to prove what Prodigal Regeneration (or its suppression) is actually contributing. Noisy: one line per vital per ~5s per player below max. Default OFF - zero overhead when off. Toggle live: /modifybool regen_diag_verbose true");
        public static ConfigProperty<bool> landblock_lifecycle_diag_verbose { get; private set; } = new(false, "If TRUE, emits WARN-level [LbLife] traces for landblock lifecycle: CONSTRUCT (instance created, physics built, walkable), INIT (spawn task started, incl. reload flag) and SPAWN-COMPLETE (objects actually landed, with count). Each line carries the landblock id AND variation. Use to catch void landblocks - a CONSTRUCT with no matching INIT/SPAWN-COMPLETE for that (landblock, variation) IS the void. Default OFF. Toggle live: /modifybool landblock_lifecycle_diag_verbose true");
        /// <summary>Variant review 2026-09-12 (item 6) test diag: WARN-level [LANDBLOCK GROUP] traces for every group add / merge / new / split with the variation, a rate-limited trace whenever a landblock tick touches an ADJACENT that sits in a different group (the cross-thread case the regrouping removes), and a 60 s summary of groups per variation plus any same-variation groups close enough that they should have merged. Toggle live: /modifybool landblock_group_diag_verbose true</summary>
        public static ConfigProperty<bool> landblock_group_diag_verbose { get; private set; } = new(false, "If TRUE, emits WARN-level [LANDBLOCK GROUP] traces: group add/merge/new/split with variation, cross-group adjacency touches (rate-limited), and a 60 s per-variation summary. Test diag for the variant grouping change. Toggle live: /modifybool landblock_group_diag_verbose true");
        public static ConfigProperty<bool> generator_diag_verbose { get; private set; } = new(false, "If TRUE, emits WARN-level [GenDiag] traces through the whole generator spawn path: InitializeGenerator loaded values (MaxGeneratedObjects/InitGeneratedObjects/GeneratorRadius/RegenerationInterval + every profile's probability/wcid/init/max), SelectAProfile enqueue decisions, GetSpawnObjectsForProfile slot math, and every object actually spawned WITH THE GENERATOR THAT SPAWNED IT. Answers 'is the server reading the values I put in the DB, and who spawned this mob'. Default OFF -- the whole path incl. string building is skipped when off. Pair with generator_diag_wcid. Toggle live: /modifybool generator_diag_verbose true");
        public static ConfigProperty<long> generator_diag_wcid { get; private set; } = new(0, "With generator_diag_verbose: trace ONLY the generator with this weenie class id (0 = every generator on the server, very noisy). Set live: /modifylong generator_diag_wcid 730000073");
        ...
        public static ConfigProperty<bool> defense_scaling_melee_enabled { get; private set; } = new(false, "if TRUE, enables dynamic scaling for melee defense to prevent extreme evasion at high skill levels");
        public static ConfigProperty<bool> defense_scaling_missile_enabled { get; private set; } = new(false, "if TRUE, enables dynamic scaling for missile defense to prevent extreme evasion at high skill levels");
        public static ConfigProperty<bool> defense_scaling_magic_enabled { get; private set; } = new(false, "if TRUE, enables dynamic scaling for magic defense to prevent extreme evasion at high skill levels");
