...
namespace ACE.Server.Managers
{
    ...
    public static class ServerConfig
    {
        ...
        public static ConfigProperty<bool> missile_power_bar { get; private set; } = new(true, "if TRUE, the missile accuracy bar works like the melee power bar: damage scales 0.5x-1.5x with the bar and hit chance is flat. If FALSE, retail behavior: bar scales attack skill (0.6x-1.6x) and damage is flat. Applies live, no restart. Use /modifybool missile_power_bar <true/false>.");
        ...
        public static ConfigProperty<long> slot_event_interval_minutes { get; private set; } = new(15, "Rotating Slot<N> events: how many minutes each slot stays active. Exactly one Slot<N> event is On at a time, chosen from the clock, so the rotation is stateless and survives restarts. 0 or fewer is treated as 1.");
        public static ConfigProperty<long> slot_event_overlap_seconds { get; private set; } = new(20, "Rotating Slot<N> events: how long the outgoing slot stays On after the incoming one starts. Generators take 5-10s to spawn and despawn, so an overlap prevents a gap where neither slot's content exists. 0 disables the overlap.");
