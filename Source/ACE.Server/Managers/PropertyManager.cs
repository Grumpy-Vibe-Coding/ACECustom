...
namespace ACE.Server.Managers
{
    ...
    public static class ServerConfig
    {
        ...
        public static ConfigProperty<double> enl_move_threshold { get; private set; } = new(8.0, "How far in units a player may end up from where they started the enlightenment ritual before it aborts. Measured horizontally between start and end only. Defaults to 8.0 to match RecallMoveThreshold, so the ritual is no easier to walk out of than a recall; it is a separate property purely so it can be tuned without touching recalls.");
