            ...
            foreach (var entry in entries.OrderByDescending(i => i.PowerLevel + (i.AugmentationLevelWhenCast ?? 0)))
            {
                ...
                else if (auggedPowerLevel == entryAuggedPowerLevel)
                {
                    ...
                    else
                    {
                        ...
                        if (!equip && caster is Player player && ((player.AugmentationIncreasedSpellDuration + player.LuminanceAugmentSpellDurationCount ?? 0) > 0 || player.GetZoneCantripBonus(ACE.Server.Managers.ZoneControl.ZoneCantrips.SpellDurationLevels) > 0))
                        {
